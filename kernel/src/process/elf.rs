...
pub fn load_elf(filename: &str) -> Result<Process, String> {
    ...
    let pml4_phys = memory::create_address_space().map_err(|e| String::from(e))?;

    x86_64::instructions::interrupts::without_interrupts(|| -> Result<(), String> {
        let (old_cr3, old_cr3_flags) = Cr3::read();
        ...
        unsafe {
            Cr3::write(old_cr3, old_cr3_flags);
        }

        Ok(())
    })?;

    let kernel_stack = memory::allocate_kernel_stack_with_guard(4).map_err(|e| String::from(e))?;
